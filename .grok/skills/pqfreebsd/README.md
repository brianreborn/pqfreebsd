...
**pqk is ideally preloaded by loader(8)** (`loader.conf.local`); later with
**full TPM e2e** on that path (not this cut). pqf skills only `kldload` as a
same-session fallback. Operators are not asked to hand-`kldload`. **pqf
skills** do not build that tree on demand (**pqk** is not LLM-dependent).
